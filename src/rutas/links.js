...
router.post('/agregar_dispositivo', isLoggedIn, async(req, res) => {
    ...
    await query('INSERT INTO PC set ?', [newDispositivo]);
...
router.get('/listar_equipos', isLoggedIn, async (req, res) => {
    const equipos = await query('SELECT * FROM PC');
...
router.get('/listar_equipos/eliminar/:id', isLoggedIn, isAux, async (req, res) => {
    const {id} = req.params;
    await query('DELETE FROM PC WHERE NOSERIE = ?', [id]);
...
router.get('/listar_equipos/editar/:id', isLoggedIn, isAux, async(req, res) => {
    const {id} = req.params;
    const equipo = await query('SELECT * FROM PC WHERE NOSERIE = ?', [id]);
...
router.post('/listar_equipos/editar/:id', isLoggedIn, isAux, async(req, res) => {
    ...
    await  query('UPDATE PC SET ? WHERE NOSERIE = ?', [newDispositivo, noserie]);
...
router.post('/usuarios/agregar_alumno', isLoggedIn, isAux, async (req, res ) => {
    ...
    await query('INSERT INTO alumno set ?', [newAlumno]);
...
router.get('/alumnos', isLoggedIn, isAux, async (req, res) => {
    const alumnos = await query('SELECT NOCONTROL, NOMBRE_AL, AP_PAT, AP_MAT, CARRERA, SEMESTRE, STATUS  FROM ALUMNO INNER JOIN CARRERA ON ALUMNO.IDCARRERA = CARRERA.IDCARRERA');
    const carreras = await query('SELECT IDCARRERA, CARRERA FROM CARRERA');
...
router.get('/alumnos/eliminar/:id', isLoggedIn, isAux, async (req, res) => {
    const {id} = req.params;
    await query('DELETE FROM ALUMNO WHERE NOCONTROL = ?', [id]);
...
router.get('/alumnos/editar/:id', isLoggedIn, isAux, async(req, res) => {
    const {id} = req.params;
    const alumnos = await query('SELECT * FROM ALUMNO WHERE NOCONTROL = ?', [id]);
    const carreras = await query('SELECT IDCARRERA, CARRERA FROM CARRERA');
...
router.post('/alumnos/editar/:id', isLoggedIn, isAux, async(req, res) => {
    ...
    await  query('UPDATE ALUMNO SET ? WHERE NOCONTROL = ?', [newAlumno, id]);
    res.redirect('/inventarios/alumnos');
});
...
router.post('/materias', isLoggedIn, isAux, async (req, res) => {
    ...
    const carreras = await query('SELECT IDCARRERA, CARRERA FROM CARRERA where idcarrera = ?', [carr]);
    const materias = await query('SELECT CLAVEMATERIA, NOMBREMAT, CARRERA, RETICULA FROM MATERIA INNER JOIN CARRERA on carrera.idcarrera = materia.idcarrera');
...
router.post('/agregar_periferico', isLoggedIn, async(req, res) => {
    ...
    await query('INSERT INTO periferico set ?', [newDispositivo]);
...
router.get('/listar_perifericos', isLoggedIn, isAux, async (req, res) => {
    const equiposmonitor = await query('SELECT * FROM PERIFERICO WHERE TIPO = "MONITOR"');
    const equiposteclado = await query('SELECT * FROM PERIFERICO WHERE TIPO = "TECLADO"');
    const equiposmouse = await query('SELECT * FROM PERIFERICO WHERE TIPO = "MOUSE"');
...
router.get('/listar_perifericos/eliminar/:id', isAux, isLoggedIn, async (req, res) => {
    const {id} = req.params;
    await query('DELETE FROM periferico WHERE noserieper = ?', [id]);
...
router.get('/listar_perifericos/editar/:id', isLoggedIn, isAux, async(req, res) => {
    const {id} = req.params;
    const perifericos = await query('SELECT * FROM periferico WHERE noserieper = ?', [id]);
...
router.post('/listar_perifericos/editar/:id', isLoggedIn, isAux, async(req, res) => {
    ...
    req.flash('exito', 'Alumno agregado con éxito');
    await  query('UPDATE periferico SET ? WHERE noserieper = ?', [newPeriferico, id]);
    res.redirect('/inventarios/listar_perifericos');
});
